...
if (isset($_POST['get_all_rooms'])) {
    ...
    while($row = mysqli_fetch_assoc($res)) {
        ...
        $room_id = $row['id'];
        $res_img = mysqli_query($con, "
            SELECT * FROM `room_images` 
            WHERE `room_id`='$room_id' 
            ORDER BY `id` ASC 
            LIMIT 1
        ");
        $thumb = mysqli_fetch_assoc($res_img);
        $thumb_img = $thumb ? $thumb['image'] : '';
        ...
        $data .= "
            <tr class='align-middle'>
                ...
                <td>
                    ...
                    <button onclick=\"room_image($row[id],'$row[name]')\" type='button' class='btn btn-dark shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#room-image'>
...
if (isset($_POST['get_room']))
{
    ...
    $res2 = select(
        "SELECT * FROM `room_features` WHERE `room_id`=?",
        [$frm_data['get_room']],
        'i'
    );
...
while($row = mysqli_fetch_assoc($res)) {
    ...
    <tr class='align-middle'>
        ...
        <td><button onclick='rem_image($row[sr_no],$row[room_id])' btn-danger btn-sm shadow-none><i class='bi bi-trash'></i></button></td>
    </tr>
    data;
    }
